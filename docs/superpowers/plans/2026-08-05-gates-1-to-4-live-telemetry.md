...
- [x] Write deterministic JUnit tests whose literal expectations prove mean/min/max/stddev/RMS, jerk, sample completeness, reset behavior, and missing-GPS serialization.
- [x] Run `mobile/android/gradlew.bat app:testDebugUnitTest --tests *ImuWindowAccumulatorTest` and verify failures because the production types do not exist.
- [x] Implement numerically stable accumulation and schema-v1 models without Android framework dependencies.
- [x] Re-run the focused test and commit `feat(android): add one-second telemetry window builder`.
...
- [x] Write in-memory Room instrumentation tests proving payload/cursor atomicity, ordered sequence assignment, lease recovery, gap-safe ACK, permanent rejection, and 24-hour ACK cleanup.
- [x] Compile the Android tests and verify production symbols are missing.
- [x] Implement entities, unique constraints, DAO transactions, WAL configuration, repository invariants, and storage-pressure status.
- [x] Compile focused unit/instrumentation suites and commit `feat(android): add durable telemetry room outbox`.
...
- [x] Write policy tests proving retry classification, bounded exponential jitter, ACK deletion boundary, and a single active uploader lease.
- [x] Verify focused tests fail before production implementation.
- [x] Implement the location foreground service, 50 Hz sensor listeners, monotonic one-second boundaries, ongoing notification, START_STICKY recovery, Keystore AES-GCM credential storage, gzip uploader, and unique WorkManager backfill.
- [x] Register the service/permissions/package and replace JavaScript `gpsTracking` ownership with the narrow native bridge.
- [x] Run Kotlin unit tests, TypeScript, ESLint, and `assembleDebug`; commit `feat(android): ship durable foreground telemetry collector`.
...
- [x] Write route tests proving offline-first start replay, end replay, finalization waiting on gaps, and device/vehicle isolation.
- [x] Run focused tests and observe missing-route failures.
- [x] Implement forward-only migration, models, and v2 lifecycle routes; finalization becomes eligible only at the declared contiguous cursor.
- [x] Run focused/full backend suites and commit `feat: add durable telemetry trip lifecycle`.
...
- [x] Write fake-adapter integration tests for relay crash boundaries, repeated delivery, independent processor idempotency, pending reclaim, poison dead-lettering, and Redis outage backlog preservation.
- [x] Verify tests fail because stream/processors do not exist.
- [x] Implement protocol-based Redis adapters, relay/consumer loops, live-state freshness, IMU quality/event rules, and deterministic finalization from committed windows.
- [x] Run focused/full tests and commit `feat: add recoverable telemetry processors`.
...
- [x] Write tests for fleet isolation, LIVE/DELAYED/OFFLINE/GPS_LOST/SYNCING/DEGRADED labels, WebSocket snapshot/version-gap recovery, coordinate-safe metrics, and manifest mismatch refusal.
- [x] Verify focused failures, implement the routes/services, then run focused/full tests.
- [x] Commit `feat: add realtime recovery and archive verification`.
...
- [x] Add executable configuration checks that reject public buckets, shared service accounts, unbounded Cloud Run database connections, missing backup/PITR, and API-replica migrations.
- [x] Run checks against absent infrastructure and observe failure.
- [x] Implement parameterized Terraform without credentials/state and process-role CLI commands.
- [x] Parse all HCL, run static topology and Compose validation, and commit `infra: add gcloud live telemetry topology`; Terraform CLI/provider validation remains external because Terraform is not installed on this workstation.
...
- [x] Write deterministic tests for rate schedules, unique sequences, live-priority fairness, SLO percentile evaluation, 50% headroom rule, and three-day cohort holds.
- [x] Verify focused failures, implement both tools and operator recovery/dead-letter/archive/rollback procedures, then run focused/full tests.
- [x] Run a short local two-identity smoke load without claiming physical or Cloud SQL certification.
- [x] Record verified repository evidence and explicitly mark company-GCP, two-real-device, 60-minute capacity, restore-drill, and three-day cohort evidence as pending until executed.
- [x] Commit `test: add telemetry certification and rollout gates`.
...
- [x] Run backend full tests, Alembic clean upgrade/current, Android unit tests, TypeScript, ESLint, debug build, Compose validation, Terraform checks when installed, and repository hygiene checks.
- [x] Audit all 18 master acceptance criteria against evidence; distinguish implemented, locally verified, and externally pending.
- [x] Update documentation and checklist states from actual output only.
- [x] Commit `docs: complete live telemetry implementation gates` and preserve the branch for user-directed integration.
